  ...
  return (
    <Dialog.Root>
      <div className="flex justify-center w-full h-full p-8 TRACKER-CONTAINER">
        <div className="flex flex-col items-center justify-center h-full p-2 text-gray-700 TRACKER-SUB-CONTAINER max-w-screen-2xl">
          ...
          <section className="grid w-full mb-4 bg-opacity-25 TOOL-BAR gap-x-2 gap-y-2">
            <div className="ADD-JOB-BUTTON-CONTAINER w-fit">
              <Dialog.Trigger className="w-full h-full">
                <div className="grid items-center w-48 h-full grid-cols-3 text-xs text-gray-700 transition-all bg-white border border-white rounded-lg ADD-JOB-BUTTON bg-opacity-10 hover:bg-opacity-25">
            ...
            <div className="grid justify-center gap-x-4 COLOR-TIPS">
              <span className="flex items-center text-xs text-gray-700 gap-x-1 whitespace-nowrap">
                <Icon className="" path={mdiCircleSlice8} size={0.8} /> AWAITING
                RESPONSE
              </span>
              <span className="flex items-center text-xs text-steel-blue gap-x-1 whitespace-nowrap">
                <Icon className="" path={mdiCircleSlice8} size={0.8} />{" "}
                INTERVIEW STAGE
              </span>
              <span className="flex items-center text-xs text-yellow-500 gap-x-1 whitespace-nowrap">
                <Icon className="" path={mdiCircleSlice8} size={0.8} /> OFFER
                RECEIVED
              </span>
            ...
            <div className="flex items-center w-full rounded-lg SEARCH-BAR">
              <div className="flex items-center pl-2 pr-2 bg-black bg-opacity-50 rounded-lg SEARCH-BAR-CONTAINER ">
                <Icon
                  className="text-gray-700 text-opacity-25"
                  path={mdiMagnify}
                  size={0.8}
                />
                <input
                  className="w-48 h-8 p-1 text-sm text-gray-700 transition-all bg-black bg-opacity-0 SEARCH-BAR-INPUT"
                  type="text"
            ...
            <div className="flex justify-between w-full mt-2 select-none PaginateContainer">
              <div className="flex items-center text-xs text-gray-700">
                {jobApps && jobApps.length > 0 ? (
                  <span>
                    Showing {paginate.showingA} to{" "}
                    {paginate.showingB > paginate.totalCount
                      ? paginate.totalCount
                      : paginate.showingB}{" "}
                    of {paginate.totalCount}
                  </span>
                ) : (
              ...
              <ReactPaginate
                ...
                ${
                  !jobApps &&
                  "pointer-events-none text-gray-500 text-opacity-30"
                }
                ${
                  paginate.page === 1 &&
                  "pointer-events-none text-gray-500 text-opacity-30"
                ...
                ${
                  paginate.page * paginate.pageSize >= paginate.totalCount &&
                  "pointer-events-none text-gray-500 text-opacity-30"
                }
                `}
                activeClassName={`PaginatePageActive bg-white text-gray-700 bg-opacity-30 p-1 flex justify-center w-6 flex
          ...
          <section className="w-full">
            <button
              className="p-1 text-xs text-gray-700 transition-all bg-green-800 border border-green-500 rounded-lg bg-opacity-10 w-44 hover:bg-opacity-25"
